...
: Set/reset by the <code>enable</code> command. Enables ntp clock discipline.
...
The <code>pps_update</code> bit isset in the protocol routine if the prefer peer has survived and has offset less than 128 ms; otherwise set to zero.

The <code>pps_control</code> bit is updated to the current time by kernel support if the PPS signal is enabled and working correctly. Set to zero in the adjust routine if the interval since the last update exceeds 120 s.
