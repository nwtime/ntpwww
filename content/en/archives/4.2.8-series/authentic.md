...
Keys and related information are specified in a keys file, which must be distributed and stored using secure means beyond the scope of the NTP protocol itself. Besides the keys used for ordinary NTP associations, additional keys can be used as passwords for the <code>[ntpq](/archives/4.2.8-series/ntpq)</code> and <code>[ntpdc](/archives/4.2.8-series/ntpdc)</code> utility programs. Ordinarily, the <code>ntp.keys</code> file is generated by the <code>[ntp-keygen](/archives/4.2.8-series/keygen)</code> program, but it can be constructed and edited using an ordinary text editor.
