...
For the purposes of the following description, call the last selected system peer the _old peer_, and the currently selected source the _candidate peer_. At each update, the candidate peer is selected as the first peer on the survivor list sorted by increasing root distance. The algorithm initializes the -_clockhop threshold_ with the value of <code>mindist</code>, by default 1 ms.
...
5.  When the Automated Computer Time Service driver (type 18) is configured, it is designated the _modem driver_. It is used either as a backup source, should all other sources fail, or as the primary source if the <code>prefer</code> option is present.
